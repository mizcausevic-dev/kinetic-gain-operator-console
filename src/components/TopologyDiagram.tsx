...
import { TopologyNode, TopologyLink, SpecType } from '../types';
import { 
  Activity, 
  Cpu, 
  Layers, 
...
export default function TopologyDiagram({ onSelectSpec, onAddLog }: TopologyDiagramProps) {
  ...
  const [links, setLinks] = useState<TopologyLink[]>(initialTopologyLinks);
  ...
  const triggerManualDiagnostics = () => {
    ...
    // Create pulses for all outbound links from this node
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
      ...
      <div className="lg:col-span-3 bg-[#07080c] border border-slate-800 rounded p-4 relative overflow-hidden flex flex-col justify-between select-none" style={{ minHeight: '520px' }}>
        ...
        {/* Header toolbar */}
        <div className="flex flex-wrap items-center justify-between gap-4 z-10 border-b border-slate-800 pb-3 mb-3">
          <div>
            ...
            <p className="text-[10px] text-slate-500 mt-0.5">Interactive network diagram visualizing open JSON specifications transit routes</p>
          </div>

          <div className="flex items-center gap-1.5">
            <button
              onClick={() => setIsSimulating(!isSimulating)}
              className={`px-2 py-1 rounded-sm text-[10px] font-mono font-bold transition-all flex items-center gap-1 border ${
                isSimulating 
            ...
            <button
              onClick={() => {
                ...
                onAddLog(`Topology visualizer latency heatmap ${!heatmapActive ? 'ENABLED' : 'DISABLED'}.`, 'aeo', 'valid');
              }}
              className={`px-2 py-1 rounded-sm text-[10px] font-mono font-bold transition-all flex items-center gap-1 border ${
                heatmapActive 
                  ? 'bg-amber-950/40 border-amber-600/30 text-amber-400 hover:bg-slate-900 shadow-sm shadow-amber-500/10' 
                  : 'bg-slate-900 border-slate-850 text-slate-400 hover:bg-slate-850'
              }`}
              title="Toggle bottleneck latency color heatmap"
            >
              <Zap className={`h-3 w-3 ${heatmapActive ? 'text-amber-400 animate-pulse' : 'text-slate-400'}`} />
              HEATMAP
            </button>
            <button
              onClick={() => {
              ...
              className="p-1.5 bg-slate-900 border border-slate-800 hover:bg-slate-850 rounded-sm text-slate-550 font-mono text-[10px] hover:text-red-400"
            >
        ...
        <div className="relative flex-1 bg-[#040508] border border-slate-800 rounded-sm overflow-hidden flex items-center justify-center p-2" style={{ minHeight: '360px' }}>
          
          {/* Overlay connection specs guide */}
          <div className="absolute top-3 left-3 flex flex-col gap-1.5 z-10 pointer-events-none hidden sm:flex bg-black/70 p-2 rounded border border-slate-850/40 backdrop-blur-xs select-none shadow-md">
            {!heatmapActive ? (
              <>
                ...
                <div className="flex items-center gap-2 text-[10px] font-mono text-emerald-400">
                  <span className="w-2.5 h-1 bg-emerald-400/80 rounded-full inline-block"></span> AEO Card Flow
                </div>
                <div className="flex items-center gap-2 text-[10px] font-mono text-purple-400">
                  <span className="w-2.5 h-1 bg-purple-400/80 rounded-full inline-block"></span> Prompt Provenance
                </div>
                <div className="flex items-center gap-2 text-[10px] font-mono text-pink-400">
                  <span className="w-2.5 h-1 bg-pink-400/80 rounded-full inline-block"></span> AI Evidence Proof
                </div>
                <div className="flex items-center gap-2 text-[10px] font-mono text-rose-400">
                  <span className="w-2.5 h-1 bg-rose-400/80 rounded-full inline-block"></span> Classroom AI AUP
                </div>
              </>
            ) : (
              <>
                <span className="text-[10px] font-mono text-amber-400 uppercase tracking-widest font-semibold border-b border-amber-805 pb-1">Heatmap Legend</span>
                <div className="flex items-center gap-2 text-[10px] font-mono text-emerald-450">
                  <span className="w-2.5 h-1 bg-emerald-500 rounded-full inline-block"></span> Nominal (&lt; 80ms)
                </div>
                <div className="flex items-center gap-2 text-[10px] font-mono text-amber-400">
                  <span className="w-2.5 h-1 bg-amber-500 rounded-full inline-block"></span> Warning (80-200ms)
                </div>
                <div className="flex items-center gap-2 text-[10px] font-mono text-rose-450">
                  <span className="w-2.5 h-1 bg-rose-500 rounded-full inline-block"></span> Bottleneck (&gt; 200ms)
                </div>
                <div className="text-[7px] font-mono text-slate-500 mt-1 max-w-[125px] leading-snug">
                  *Click any link latency badge to spike lag and test SRE compliance warnings.
                </div>
              </>
            )}
          </div>

          <svg className="w-full h-full min-h-[350px]" style={{ touchAction: 'none' }} viewBox="0 0 100 100" preserveAspectRatio="none">
            {/* Draw pathways */}
            {links.map(link => {
              ...
              // Color coordinate pathways by spec
              let strokeColor = '#475569'; // Default charcoal slate-600
              if (link.activeSpec === 'aeo') strokeColor = 'rgba(52, 211, 153, 0.45)';
              else if (link.activeSpec === 'prompt_provenance') strokeColor = 'rgba(167, 139, 250, 0.45)';
              else if (link.activeSpec === 'ai_evidence') strokeColor = 'rgba(244, 114, 182, 0.45)';
              else if (link.activeSpec === 'classroom_aup') strokeColor = 'rgba(251, 113, 133, 0.45)';
              else if (link.activeSpec === 'student_disclosure') strokeColor = 'rgba(34, 211, 238, 0.45)';

              const latency = linkLatencies[link.id] || 50;
              let heatmapColor = 'rgba(16, 185, 129, 0.85)'; // emerald
              if (latency >= 80 && latency < 200) {
                heatmapColor = 'rgba(245, 158, 11, 0.9)'; // amber
              } else if (latency >= 200) {
                heatmapColor = 'rgba(239, 68, 68, 0.95)'; // crimson
              }

              const targetColor = heatmapActive ? heatmapColor : strokeColor;
              ...
              return (
                <g key={link.id}>
                  {/* Outer glowing warning back-layer when high latency under heatmap modes */}
                  {heatmapActive && latency >= 200 && (
                    <line
                  ...
                  <line
                    ...
                    y2={dest.coordinates.y}
                    stroke={targetColor}
                    strokeWidth={heatmapActive ? (latency >= 200 ? 1.4 : 0.8) : (isLinkActive ? 1.0 : 0.45)}
                    strokeDasharray={heatmapActive ? undefined : (isLinkActive ? "1,0" : "1.5,1.5")}
                  ...
                  {heatmapActive && (
                    <g 
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        // Cycle/Spike latency: Spikes +115ms each click, wraps around 500ms
                        setLinkLatencies(prev => {
            ...
            {pulses.map(pulse => {
              ...
              // Linear interpolation of coordinate path
              ...
              let pulseColor = '#a78bfa';
              if (pulse.spec === 'aeo') pulseColor = '#10b981';
              else if (pulse.spec === 'prompt_provenance') pulseColor = '#8b5cf6';
              else if (pulse.spec === 'ai_evidence') pulseColor = '#ec4899';
              else if (pulse.spec === 'classroom_aup') pulseColor = '#f43f5e';
              else if (pulse.spec === 'student_disclosure') pulseColor = '#06b6d4';
            ...
            {nodes.map(node => {
              ...
              return (
                ...
                >
                  {/* Ripple radial animation shadow around active selection */}
                  {isActive && (
                    <circle
                      r="4.5"
                      fill="none"
                  ...
                  {/* Node solid frame base */}
                  <circle
                    r="2.8"
                    fill="#0f172a" // slate-900
                    stroke={isActive ? '#ffffff' : themeColor}
                    strokeWidth={isActive ? '0.4' : '0.2'}
                    style={{ transition: 'stroke 0.3s ease, stroke-width 0.3s' }}
                  />
                  ...
                  {/* Mini-Label for text inside SVG rendering */}
                  <text
                    y="5.5"
                    textAnchor="middle"
                    ...
                    className="font-mono tracking-wide pointer-events-none select-none bg-slate-950"
                  >
                    {node.label.split(' ')[0]}
                  </text>
                </g>
          ...
          <div className="absolute bottom-2.5 right-2.5 font-mono text-[9px] text-slate-500 bg-black/80 px-1.5 py-0.5 rounded-sm border border-slate-800/80 pointer-events-none">
            PULSES SCALED: {pulses.length} FRAME/SEC | SYS CHANNELS: ACTIVE_FLUX
          </div>
        </div>
      </div>
          ...
          <div className="flex-1 flex flex-col justify-between">
            <div>
              ...
              <div className="grid grid-cols-1 gap-2 mt-3 bg-black/30 p-2.5 rounded border border-slate-800/60 font-mono">
                <span className="text-[8px] text-slate-550 uppercase tracking-wider font-bold">Real-Time Telemetry</span>
                ...
                <div className="space-y-1">
                  <div className="flex justify-between text-[10px]">
                    ...
                    <span className={activeNode.metrics.cpu > 70 ? 'text-amber-400 font-bold' : 'text-slate-350'}>
                ...
                <div className="flex justify-between items-center text-[9.5px] border-t border-slate-800/40 pt-1.5">
                  <span className="text-slate-450 flex items-center gap-1">
                    <Layers className="h-3 w-3 text-blue-455" />
                    VRAM/MEM FOOTPRINT
                  </span>
                  <span className="text-slate-350 text-[10px]">{activeNode.metrics.memory}</span>
                </div>
              </div>
              ...
              <div className="mt-4 space-y-3 font-mono">
                {activeNode.specsProduced.length > 0 && (
                  <div>
                    <h4 className="text-[8px] text-slate-550 uppercase tracking-wider font-bold mb-1 flex items-center gap-1">
            ...
            <div className="pt-4 border-t border-slate-800 mt-4 space-y-2">
              ...
              <div className="flex gap-1.5 text-[9px] text-slate-550 leading-normal p-2 bg-black/45 rounded-sm border border-slate-800/85">
